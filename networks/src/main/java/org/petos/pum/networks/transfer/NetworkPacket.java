...
private NetworkExchange.ResponseType responseType;
private NetworkExchange.RequestType requestType;
...
public NetworkPacket(NetworkExchange.RequestType type, int requestCode) {
      this.requestType = type;
      this.direction = PacketDirection.Request;
      this.packetCode = requestCode;
}

public NetworkPacket(NetworkExchange.RequestType type, int requestCode, byte[] data) {
      this(type, requestCode);
      setPayload(data);
}

public NetworkPacket(NetworkExchange.ResponseType type, int responseCode, byte[] data) {
      this(type, responseCode);
      setPayload(data);
}

public NetworkPacket(NetworkExchange.ResponseType type, int responseCode) {
...
/**
 * Payload is not effective data. But additional info for NetworkPacket (any kind of DTO)
 * @return size of attached payload
 */
...
public @NotNull String stringData() {
      return BinaryObjectMapper.toString(data);
}
...
public @NotNull String stringFrom(int offset) {
      ...
      if (BUFF_LENGTH != 0 && data.length != 0) {
	    ...
	    result = BinaryObjectMapper.toString(payload);
      }
      return result;
}
...
public static Optional<NetworkPacket> valueOf(byte[] rawBytes) {
      ...
      if (controlSign == CONTROL_REQUEST) {
	    ...
	    packet = new NetworkPacket(NetworkExchange.RequestType.values()[enumId], code);
      } else {
	    int enumId = typeSign & RESPONSE_CONTROL_MASK;
	    int code = (typeSign & RESPONSE_CODE_MASK) >>> 1;
	    packet = new NetworkPacket(NetworkExchange.ResponseType.values()[enumId], code);
      }
...
private static @NotNull byte[] getRawPacket(NetworkPacket packet, NetworkExchange.ResponseType type) {
...
private static @NotNull byte[] getRawPacket(NetworkPacket packet, NetworkExchange.RequestType type) {
      int payloadSize = packet.payloadSize();
      byte[] payload = packet.data();
